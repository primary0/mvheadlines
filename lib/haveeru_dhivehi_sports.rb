class HaveeruDhivehiSports < NewsAgency
  ...
  def get_pages
    begin
      @news_page = Hpricot(open('http://www.haveeru.com.mv/?page=listcategory&cat=urwviLuk'))      
      true
    rescue
  ...
  def get_links
    begin
      ...
      begin
        ...
        log_error("#{name} - Sports", "Extracting sports cover story")
      end
      
      # more sports links at the bottom
      begin
        ...
        log_error("#{name} - Sports", "Unable to extract more new links")      
      end
      ...
      begin
        ...
        log_error("#{name} - Sports", "Unable to extract links on the left side")
      end  
